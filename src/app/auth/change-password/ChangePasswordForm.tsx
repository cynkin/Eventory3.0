...
import {updatePassword} from "@/server-components/auth/update-password";
            ...
            <form onSubmit={handleSubmit} className="w-full " method="post">
                ...
                <ul className="text-sm text-gray-700 list-disc pl-5 space-y-1 mb-6">
                    ...
                    {(!/\d/.test(password) || !/[a-zA-Z]/.test(password)) &&
                        <li className={listStyle}>Combines letters and numbers</li>}
                    {!(/[\W_]/.test(password)) &&
                        <li className={listStyle}>A special character ~ # @ $ % & ! * _ ? ^ -</li>}
